...
Shift_Add:
    xor dx, dx           ; Clear DX (result accumulator)
    mov al, [num1]       ; Load multiplicand into AL
    mov bl, [num2]       ; Load multiplier into BL
    mov cl, 8            ; Loop 8 times (for 8 bits)

shift_loop:
    bt bx, 0             ; Test bit 0 (LSB) of multiplier
    jnc skip_add         ; If 0, skip addition
    add dx, ax           ; Add multiplicand to result

skip_add:
    shl ax, 1            ; Shift multiplicand left (for next bit)
    shr bl, 1            ; Shift multiplier right (to bring next bit to LSB)
    loop shift_loop      ; Decrement CL and repeat

    mov bx, dx           ; Move result to BX for display
    call Hex_to_Ascii    ; Convert result to ASCII
    ret
            ; Add AX to CX (accumulate result)
